...
public sealed class HermiteSegmentFactory : IBoundaryElementFactory2D
{
    ...
    private static IBasisFunction1D[] CreateObliqueBasis()
    {
        return
        [
            SegmentBasis.N0,
    ...
    // костыль, но пока так 
    public sealed class Dof(BoundaryMode mode) : DofManager(
        dofCount: mode == BoundaryMode.Oblique ? 2 : 4
    )
    {
        private readonly BoundaryMode _mode = mode;

        public override int NumberOfDofOnVertex => _mode == BoundaryMode.Oblique ? 1 : 2;
        ...
        public override void SetVertexDof(int localVertexIndex, int n, int dofIndex)
        {
            ...
            if (_mode == BoundaryMode.Oblique)
            {
                Debug.Assert(n == 0);
                _dof[localVertexIndex] = dofIndex; // только u
                return;
            }
            ...
            _dof[pos] = (_mode == BoundaryMode.Horizontal) ? dofIndex : dofIndex + 1;
        }
