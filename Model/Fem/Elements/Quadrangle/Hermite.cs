...
{
    public static Polynomial2D CreatePoly(
        Polynomial2D[] refBasis, // Все 16 функций элемента
        double[] coefficients)   // Столбец матрицы трансформации T
    {
        var poly = new Polynomial2D();
...
public sealed class HermiteQuadrangleFactory : IFiniteElementFactory2D
{
    public IFiniteElement2D CreateElement(IMesh2D mesh, int[] vertices, int materialIndex)
    {
        ...
        var physicalBasis = new IPolynomial2D[16];
        ...
        for (int k = 0; k < 4; k++)
        {
            ...
            cU[idx[0]] = 1.0;
            physicalBasis[baseIdx + 0] = PhysicalHermiteBasis2D.CreatePoly(refBasis, cU);
            ...
            cUx[idx[2]] = x_eta;
            cUx[idx[3]] = x_xieta;
            physicalBasis[baseIdx + 1] = PhysicalHermiteBasis2D.CreatePoly(refBasis, cUx);
            ...
            cUy[idx[2]] = y_eta;
            cUy[idx[3]] = y_xieta;
            physicalBasis[baseIdx + 2] = PhysicalHermiteBasis2D.CreatePoly(refBasis, cUy);
            ...
            physicalBasis[baseIdx + 3] = PhysicalHermiteBasis2D.CreatePoly(refBasis, cUxy);
        }
